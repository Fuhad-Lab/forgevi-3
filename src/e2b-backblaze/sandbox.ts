/**
 * Forgevi 3.0 — the sandbox abstraction.
 *
 * TWO implementations, ONE interface:
 *   - E2B   : production cloud sandboxes (E2B_API_KEY set)
...
export interface SandboxAdapter {
  ...
  /** Stop the sandbox. Local: no-op (the directory persists). */
...
const E2B_SANDBOX_TIMEOUT_MS = 55 * 60_000; // hobby-tier cap with margin
...
export class E2BSandboxAdapter implements SandboxAdapter {
  ...
  static async create(template: string | undefined): Promise<E2BSandboxAdapter> {
    const { Sandbox } = (await import("e2b")) as { Sandbox: { create: (opts: Record<string, unknown>) => Promise<E2BSandbox> } };
    ...
    return new E2BSandboxAdapter(sandbox, sandbox.sandboxId);
  }
